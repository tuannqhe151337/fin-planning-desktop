...
export const UpdateCurrencySchema: ZodType<FormData> = z.object({
...
interface Props {
  ...
  show: boolean;
  onClose: () => any;
  onEditSuccessfully?: () => any;
}

export const CurrencyEditModal: React.FC<Props> = ({
  currency,
  show,
  onClose,
  ...
  const {
    ...
    resolver: zodResolver(UpdateCurrencySchema), // Apply the zodResolver
    values: {
  ...
  const [updateCurrency, { isSuccess, isLoading, isError, error }] =
    useUpdateCurrencyMutation();
  ...
  useEffect(() => {
    if (!isLoading && isSuccess) {
      toast("Update currency successfully!", { type: "success" });
