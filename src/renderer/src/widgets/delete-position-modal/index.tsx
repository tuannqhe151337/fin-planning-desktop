...
interface Props {
  ...
  show: boolean;
  onClose: () => any;
  onDeleteSuccessfully?: (Position: Position) => any;
}

export const DeletePositionModal: React.FC<Props> = ({
  position,
  show,
  onClose,
  ...
  useEffect(() => {
    if (!isLoading && !isError && isSuccess) {
      toast("Delete position successfully!", { type: "success" });
      onClose && onClose();
      onDeleteSuccessfully && onDeleteSuccessfully(position);
    }
