...
interface Props {
  ...
  onRoleIdChange?: (roleId: number | null | undefined) => any;
  onPositionIdChange?: (positionId: number | null | undefined) => any;
}

export const ListUserFiler: React.FC<Props> = ({
  searchboxValue,
  onSearchboxChange,
  onRoleIdChange,
    ...
    >
      <motion.div className="flex justify-end mt-4">
        <motion.div variants={childrenAnimation} className="mr-4">
          <RoleFilter
            onChange={(option) => {
              onRoleIdChange && onRoleIdChange(option?.value);
            }}
          />
        </motion.div>
        ...
        <motion.div variants={childrenAnimation} className="">
          <PositionFilter
            onChange={(option) => {
              onPositionIdChange && onPositionIdChange(option?.value);
            }}
          />
        </motion.div>
      </motion.div>
